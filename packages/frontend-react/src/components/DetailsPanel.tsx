...
export function DetailsPanel({
  ...
  const [preview, setPreview] = useState<{ body: ReactNode; x: number; y: number } | null>(null);
  ...
  const handleRenderedMouseOut = (ev: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (
      related &&
      (previewAnchorRef.current.contains(related) || previewComponentRef.current?.contains(related))
    )
      return;
    hidePreview();
  };

  const showPreview = async (anchor: HTMLAnchorElement, ev: React.MouseEvent<HTMLDivElement>) => {
  ...
  return (
    <>
      ...
      <div
        id="offcanvasDetails"
        className={`offcanvas offcanvas-end responsive-wide ${open ? "show" : ""}`}
        tabIndex={-1}
