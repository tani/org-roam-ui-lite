...
function isErrnoExceptionWithCode(
	errorToCheck: unknown,
	code: string,
): errorToCheck is NodeJS.ErrnoException {
	return (
		errorToCheck instanceof Error &&
		"code" in errorToCheck &&
		(errorToCheck as NodeJS.ErrnoException).code === code
	);
}
	...
	return new Promise((resolve, reject) => {
		checker.init(options, (err, pkgs) => {
			if (err) {
				...
				reject(
					new Error(
						"license-checker returned undefined packages without an error.",
					),
				);
			}
		});
...
async function main() {
	try {
		...
		for (const [src, dst] of copies) {
			try {
				...
				if (isErrnoExceptionWithCode(accessError, "ENOENT")) {
					...
					console.warn(
						`Could not access source path ${src}. Skipping. Error:`,
			...
			if (stat.isFile()) {
				...
				console.warn(
					`Skipping ${src} as it is neither a file nor a directory.`,
				);
			}
		}
		...
		const licensePackages: PackagesInfo = await checkLicensesPromise({
			start: process.cwd(),
		} as InitOpts);
		...
		for (const [packageName, licenseInfo] of Object.entries(licensePackages)) {
			if (licenseInfo.licenseFile) {
				...
				try {
					console.log(
						`Copying license for ${packageName} to ${licenseDestination}`,
					);
					...
					console.error(
						`Error copying license file ${licenseInfo.licenseFile} for ${packageName}:`,
						errorMessage,
					);
				}
			}
